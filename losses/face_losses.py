...
def arcface_loss_onehot(x_inputs, y_labels, num_classes, s=64., m=0.5, epsilon=1e-8):
    ...
    with tf.variable_scope('arcface_loss_onehot'):
        ...
        weight = tf.Variable(
            initial_value=tf.random_normal((num_classes, x_inputs_shape[1])) * tf.sqrt(2 / x_inputs_shape[1]),
        ...
        # [batch_size num_classes]
        logit_final = tf.add(tf.multiply(logit, inv_mask), tf.multiply(logit_arcface, mask), name='logit_final')
    # loss
